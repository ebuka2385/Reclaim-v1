...
describe('ApiService Integration', () => {
  ...
  describe('Messaging Operations', () => {
    it('should get conversations', async () => {
      ...
      (global.fetch as jest.Mock).mockResolvedValueOnce({
        ok: true,
        json: async () => ({ data: { conversations: mockConversations } }),
      });

      const result = await apiService.getConversations();

      expect(global.fetch).toHaveBeenCalledWith(
        `${API_BASE_URL}/messaging/conversations?userId=temp-user-id`
      );
    ...
    it('should get messages', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        `${API_BASE_URL}/messaging/threads/thread-1/messages?userId=temp-user-id`
      );
    ...
    it('should send message', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        `${API_BASE_URL}/messaging/threads/thread-1/messages`,
        expect.objectContaining({
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
