...
export default function MapScreen() {
  ...
  const loadMapPins = async () => {
    try {
      ...
      // Don't show error to user, just use empty pins
      setPins([]);
    } finally {
      setLoading(false);
    }
  ...
  // Generate HTML for a beautiful map with markers
  const mapHtml = useMemo(() => {
    ...
    // Create markers with better styling
    ...
    return `
      ...
      <head>
        ...
        <style>
          * { margin: 0; padding: 0; box-sizing: border-box; }
          ...
          /* Hide attribution on mobile for cleaner look */
      ...
      <body>
        ...
        <script>
          ...
          // Use CartoDB Positron style for a clean, modern look
          ...
          ${pins.length > 0 ? `
            const group = new L.featureGroup([${pins.map(p => `L.marker([${p.latitude}, ${p.longitude}])`).join(',')}]);
